/*
 * Tutorial: http://blog.anudeep2011.com/heavy-light-decomposition/
 ...
 */
void make_tree(int cur, int s, int e) { // 1, 0, ptr
	if(s == e-1) {
		st[cur] = baseArray[s];
		return;
	}
 ...
 */
void HLD(int curNode, int cost, int prev) {     // root, -1, -1
	if(chainHead[chainNo] == -1) {
		chainHead[chainNo] = curNode; // Assign chain head
	}
	chainInd[curNode] = chainNo; // Assign which chain curNode belongs to.
